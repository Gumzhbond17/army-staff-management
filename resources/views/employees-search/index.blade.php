...
@section('content')
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap py-2 px-3">
                    ...
                    {{-- Search --}}
                    <form action="{{ route('employees-search.index') }}" method="GET"
                        ...
                        <a href="{{ route('employees-search.index') }}"
            ...
            <div class="card-body">
                <div class="table-responsive rounded-3 border">
                        ...
                        <tbody>
                            @forelse ($provinces as $province)
                                <tr>
                                    ...
                                    <td class="fw-medium">{{ $province->code }}</td>
                                    <td class="fw-medium">{{ $province->name }}</td>
                                    <td>
                                        <span class="badge rounded-pill d-inline-flex align-items-center gap-1"
                                            style="background-color: {{ $province->is_active ? '#EAF3DE' : '#FCEBEB' }};
                                                   color: {{ $province->is_active ? '#27500A' : '#501313' }};
                                                   font-weight:500; font-size:12px;">
                                            <span class="rounded-circle"
                                                style="width:6px; height:6px; display:inline-block;
                                                       background: {{ $province->is_active ? '#3B6D11' : '#A32D2D' }};"></span>
                                            {{ $province->is_active ? 'ໃຊ້ງານ' : 'ບໍ່ໃຊ້ງານ' }}
                                        </span>
                                    </td>
                                    <td class="text-muted" style="font-size:13px; font-family:monospace;">
                                        {{ $province->created_at->format('Y-m-d H:i:s') }}
                                    </td>
                                    <td class="text-muted" style="font-size:13px; font-family:monospace;">
                                        {{ $province->updated_at->format('Y-m-d H:i:s') }}
                                    </td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            <button type="button"
                                                ...
                                                data-active="{{ $province->is_active ? '1' : '0' }}"
                                                data-url="{{ route('employees-search.update', $province->id) }}">
                                            ...
                                            <button type="button"
                                                ...
                                                data-name="{{ $province->name }}"
                                                data-url="{{ route('employees-search.destroy', $province->id) }}">
                    ...
                    <div class="mt-3 mx-3">
                        {{ $provinces->appends(request()->query())->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- ══════════════════════════════════════════
         MODAL — Create / Edit
    ══════════════════════════════════════════ --}}
    <div class="modal fade" id="province" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                <form id="provinceForm" method="POST" action="">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="provinceLabel"></h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-medium">
                                ລະຫັດແຂວງ <span class="text-danger">*</span>
                            </label>
                            <input type="text" class="form-control" name="code"
                                id="inputCode" placeholder="ປ້ອນລະຫັດແຂວງ..." required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-medium">
    ...
    <script>
        ...
        waitForBootstrap(function () {
            ...
            // ── Refs ─────────────────────────────────────────────────
            var modalEl     = document.getElementById('province');
            ...
            var labelTitle  = document.getElementById('provinceLabel');
            ...
            document.getElementById('btnCreate').addEventListener('click', function () {
                ...
                provinceForm.action      = "{{ route('employees-search.store') }}";
            ...
            document.addEventListener('click', function (e) {
                ...
                provinceForm.action      = btn.getAttribute('data-url');
                formMethod.value     = 'PUT';
                inputCode.value      = btn.getAttribute('data-code');
