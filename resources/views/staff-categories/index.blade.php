...
@section('content')
    <div class="row mt-4">
        <h4 class="mb-4"><i class="bi bi-person-bounding-box me-2"></i>ຈັດການຂໍ້ມູນປະເພດພະນັກງານ</h4>
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap py-2 px-3">

                    {{-- Search --}}
                    <form action="{{ route('staff-categories.index') }}" method="GET"
            ...
            <div class="card-body">
                <div class="table-responsive rounded-3 border">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($staff_categories as $category)
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        <span class="badge rounded-pill d-inline-flex align-items-center gap-1"
                                            style="background-color: {{ $category->is_active ? '#EAF3DE' : '#FCEBEB' }};
                                                   color: {{ $category->is_active ? '#27500A' : '#501313' }};
                                                   font-weight:500; font-size:12px;">
                                            <span class="rounded-circle"
                                                style="width:6px; height:6px; display:inline-block;
                                                       background: {{ $category->is_active ? '#3B6D11' : '#A32D2D' }};"></span>
                                            {{ $category->is_active ? 'ໃຊ້ງານ' : 'ບໍ່ໃຊ້ງານ' }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <button type="button"
                                                ...
                                                data-url="{{ route('staff-categories.update', $category->id) }}">
                                            ...
                                            <button type="button"
                                                ...
                                                data-name="{{ $category->name }}"
                                                data-url="{{ route('staff-categories.destroy', $category->id) }}">
                    ...
                    <div class="mt-3 mx-3">
                        {{ $staff_categories->appends(request()->query())->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        waitForBootstrap(function () {
            ...
            var labelTitle  = document.getElementById('categoryModalLabel');
            ...
            document.getElementById('btnCreate').addEventListener('click', function () {
                ...
                categoryForm.action      = "{{ route('staff-categories.store') }}";
            ...
            document.addEventListener('click', function (e) {
                ...
                categoryForm.action      = btn.getAttribute('data-url');
